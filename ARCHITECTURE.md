...
- `Nudge/Services/CalendarService.swift` — Apple Calendar / Canvas iCal import; rolling 21-day window extends one day per day, incremental past the `calendarSyncedThrough` cursor. Stamps deterministic stakes (`inferStakes` — no AI) and the event's real duration into `estimatedMinutes` (capped; nil for all-day).
