...
- `Nudge/Views/Tabs/TasksTabView.swift` — The biggest file. Layout order (Aug 2026): header (no description line) → `TasksMessageBox` → plan controls + timeline → Start Session → Completed → sections. Task/event list + `TodayTimelineView`; the **deterministic `planMyDay()` engine** (wake+30 → bed−60 window, fills `BusyWindowResolver` gaps around events and existing placements, 1-hour pre-event "get ready" buffer, max 4 placements, 15-min spacing, never overwrites a manual placement); the numbered drag-reorderable "Today's plan" section (`movePlanTasks` rewrites `sequenceIndex`); `clearPlan()` (removes **only** auto placements); AI Refine (its rationale renders through the message box's fifth state since Aug 2026 — the standalone banner is gone); placement/editor/`IdleStartConfirmationSheet`/`SessionTaskPickerSheet`/`WeeklyCompletedSheet`. (`TaskSortComparator` moved to `Nudge/Models/NudgeTask.swift` Jul 2026, ending the service-on-view dependency where `NudgeArbiter` and `NudgeNotificationService` consumed a type defined in this view file.)
