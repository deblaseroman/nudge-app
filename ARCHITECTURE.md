...
- **New-kind checklist:** the enum, `successIsObservableInApp` (exhaustive
  switch — compiler-checked), `TodayTimelineView.markerLabel` (likewise),
  and if it has buttons `NudgeNotificationCategoryID` + the array passed to
...
- `Nudge/ContentView.swift` — Root router (onboarding vs `MainTabView`); deep links, scene phase, the debounced `reevaluate`, and the foreground sweep (`AppOpenLog` stamp → classifier → DEBUG dump). Foreground ordering: purge → `PlacementRollover` → `ExamPrepSweep` → arbiter → calendar refresh → auto-plan → arbiter again if placed.
...
- `Nudge/Views/Tabs/HomeTabView.swift` — Brain-dump chat → `ClaudeService.sendChat` → task writes → arbiter. Owns plan supersession and the capture write rules: **DUE vs START** (`dueKind: "deadline"` → `dueDate`/`specificTime`; anything else with a date → `intendedDate` + manual placement when a time was stated + 1-hour default length via `NudgeConfig.defaultTimedIntentMinutes`); timeless events stay events; floaters require `!isEvent` and no date; a bare due date resolves to **23:59, not midnight**. `isPlanIntent()` routes "plan my day"-shaped messages to `DayPlanRefiner` instead of capture. Every model-returned item that doesn't become a row is logged with a reason and reconciled (`CAPTURE: returned/inserted/dropped`); the post-insert save is a real do/catch that tells the user on failure. The catch's error copy states only what's known — the sole throwing call precedes every insert, so an error bubble means zero tasks were created.
- `Nudge/Views/Tabs/TasksTabView.swift` — The biggest file. Layout: header → `TasksMessageBox` → Today label (+ Clear plan) → timeline → Plan my day + Start Session row → tab strip → one list at a time. Four tabs: **Unscheduled** (open tasks not placed today; includes stale non-today placements so nothing vanishes; prep tasks collapse per exam), **Today** (numbered drag-reorderable plan list + placed-today rows; empty state = Plan my day button), **Events** (two lenses: This week ≤7 days, Important = high stakes), **Overdue** (past due, open, `stakes != .low`; red badge). Today's rows are slot-tinted (`TaskRowView.slotTint`, nil in every other tab) and placed rows sort by `plannedStartDate`, not the comparator — the list must read in the same order the strip above draws. Completed rows sink to the bottom with greyed tints; `scheduledTasks` builds off `actionableTasks` so completed placements stay visible. `movePlanTasks` renumbers by display order. `planMyDay()` delegates to `DayPlanEngine` as a replan; `clearPlan()` is the undo — deliberately separate. `TaskEditorSheet` has a Duration chip section (writes `estimatedMinutes`, guarded by `durationUserPicked` — only a tapped chip writes on save; a hand-set EVENT duration also feeds `BusyWindowResolver.recordDuration`). Intent-only rows show a muted day label — never a countdown or urgency color.
- `Nudge/Views/Tabs/CalendarTabView.swift` — Calendar connect + screenshot import; calls `CalendarService` / `ScreenshotCalendarImporter`. The link source is generic "Calendar Link (iCal)" (Sep 2026) — Canvas, Google, Outlook, any `.ics` feed through one path — with an in-UI format note (link must be an iCal address, per-service directions, "a picture goes in Add from Screenshot"). Legacy `calendarSource == "Canvas iCal"` values map forward via `normalizedSource`.
...
- `Nudge/Services/ExamPrepSweep.swift` — Generates daily study tasks from exam events (lead band 3/7/14 via `prepLeadDays`) and expands captured commitments (`commitmentPhase`: shape + answered numbers → `NudgeCommitment` row + dailies; the parent capture task is deleted). Idempotent by (parent, day-stamp); `PrepTombstone` blocks recreation of user-deleted days; a user's own study task blocks the exam half entirely. Every generated daily is due **23:59 of its own day** (`sessionDueDate`). Start-day (today vs tomorrow) is decided app-side from room/growth arithmetic — the model only maps the user's answer. Split work generates once (front-loaded, half-hour blocks); rate/quantity top up a rolling 14-day window; quantity carries missed units forward, capped. Runs on launch/scene-active before the arbiter, and after capture turns.
- `Nudge/Services/DayPlanEngine.swift` — **The deterministic Plan-my-day engine.** Window via `DayWindow`, fills gaps around events/placements (1h pre-event buffer), max 4 placements (≤2 generated), 15-min spacing, never overwrites a manual placement. Candidate order: plan tasks (sequence) → **tasks intended today** (the user decided the day; the planner places, it doesn't re-decide) → score-ranked. Generated dailies place only on their own day. Displacement: at most ONE auto placement per run, only when simulation shows the gap opens and the evictee is strictly lower-stakes; displaced tasks return to Unscheduled, named in the announcement; events and manual placements are never evicted. Placement respects `TaskTimeWindow` bands. `autoPlanIfNewDay` runs once per calendar day on first foreground (marker stamped even on refusal).
- `Nudge/Services/DayPlanner.swift` — **Dead code.** The real planner is `DayPlanEngine`.
...
- `Nudge/Services/CalendarService.swift` — Apple Calendar / iCal-feed import (Canvas, Google, Outlook — any `.ics`; `webcal://` rewrites to https; non-iCal content is rejected with a format message instead of importing zero). Rolling 21-day window extends one day per day, incremental past the `calendarSyncedThrough` cursor. Fully deterministic — `inferStakes` and real durations stamped with **no AI call**; import cost is zero tokens by design.
...
- `Nudge/Services/TaskScheduler.swift`, `SmartNotificationEngine.swift`, `NotificationMessageGenerator.swift`, `FocusSessionManager.swift` — Cleared legacy stubs. Don't add code to them.
