...
- `Nudge/Services/ExamPrepSweep.swift` — Turns exam-category calendar events into daily study tasks (Aug 2026, `DESIGN.md` "Study tasks from exam events"): on launch and scene-active (called from `ContentView` beside `PlacementRollover`, BEFORE the arbiter reevaluate), each open exam event within its `prepLeadDays` band (3/7/14 off `NudgeTask.prepLeadDays`, default `NudgeConfig.defaultPrepLeadDays`) gets one `source == "prep"` task per remaining day up to (not including) the exam day — title "Study for <exam>", category exam, stakes high via `setStakesFromAutomation`, `estimatedMinutes` from the exam category prior, `linkedEventId` back to the event. Idempotent by (exam, day-stamp) against existing prep tasks; a user-made study task for the exam (deterministic `userStudyTaskMatch`: course token, or study-word + subject-token overlap) blocks creation entirely; `PrepTombstone` days are never recreated. Creation writes an announcement context (App Group) the Tasks message box shows. The decision core (`decide`) is a pure nonisolated static over value inputs so the fixture harness runs the shipped logic without SwiftData. Prep tasks are ordinary tasks downstream (morning prompt, `.prep` nudges, planners) — EXCEPT `buildDueSoonCandidates`, which excludes `source == "prep"`: their per-day due dates are self-imposed scaffolding, not deadlines.
- `Nudge/Services/DayPlanEngine.swift` — **The deterministic Plan-my-day engine** (cycle 2026-08-02-02, extracted from `TasksTabView.planMyDay()` so planning no longer needs the view): window via `DayWindow.resolve`, fills `BusyWindowResolver` gaps around events and existing placements, 1-hour pre-event "get ready" buffer, `planMaxPlacementsPerRun` (4) placements max with `planMaxPrepPlacementsPerRun` (2) of them prep, 15-min spacing, never overwrites a manual placement. **A `source == "prep"` task is a candidate only on its own study day** (cycle 2026-08-02-03 — exam-category scoring let tomorrow's block win today's gaps, un-spreading the sweep's whole point; ordinary dated tasks stay eligible early deliberately). **Placement respects `TaskTimeWindow`** (same cycle): each candidate's `effectiveTimeWindow` clamps the gap search to its band's hours (`NudgeConfig.daytime*/businessHours*`; businessHours also requires a weekday) — no in-band gap ⇒ left unplaced, never placed out-of-band, and the trace distinguishes "only out-of-band gaps" from "day full". `planToday(clearAutoFirst:)` is the button path (replan); `autoPlanIfNewDay` is the **morning auto-run** — first foreground of a calendar day (App Group day marker `nudge.autoPlan.lastRunDay`, stamped even on refusal: one attempt per day), skipped entirely if today already has auto placements, announces a placement via `PlanOutcomeContext(.planned, isAuto: true)` and stays silent on refusals. Called from `ContentView`'s scene-active Task **after** the awaited calendar refresh (ordering: purge → rollover → prep sweep → arbiter → calendar refresh → auto-plan → arbiter again if placed).
...
- `Nudge/Models/PrepTombstone.swift` — Durable record that the user DELETED a generated study task (Aug 2026): one row per (exam, day) pair, written by `ExamPrepSweep.recordDeletionIfPrep` from the Tasks tab's two real deletion sites (the DEBUG wipe-everything helper deliberately doesn't tombstone). Survives the task's removal — which is the point: `ExamPrepSweep` never recreates a tombstoned day. `noteShownAt` backs the message box's one-time "still want study time for this exam?" note (per exam: later tombstones for an already-noted exam insert pre-stamped). Registered in all three schema lists AND the widget's `membershipExceptions` in project.pbxproj.
