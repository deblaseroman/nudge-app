...
- `Nudge/Services/PlacementRollover.swift` — Day-rollover sweep clearing placements dated before today, **manual included** (a placement is a slot on one specific day). Also the **Skipped counter** (Roman, Sep 16 2026): a cleared open single task with no due date and no parent (`isSkipCandidate`) gets `skipCount += 1`; the first skip re-intends it for today (one reschedule), the second releases its intent so it lives in the Skipped tab. Subtasks and owed work are never counted. Keeps the invariant: a non-nil placement is always today-or-later. Runs before the arbiter on launch/scene-active.
