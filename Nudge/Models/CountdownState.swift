...
//  Nudge
//
//  Pure countdown/date-line state computation — no UI, no clock
//  dependency. Rendered app-side by `CountdownLabel` and consumed by
//  `WidgetCountdownFormatter`, which until Jul 2026 hand-copied this
//  logic (with a 72h days-away boundary where this file uses 48h, so the
//  widget disagreed with both the app AND its own remainingLine).
//
//  Lives in Models/ because the widget target compiles Models only —
//  moving it here is what lets both targets share one implementation of
//  the display rules, the night-suppression window, ordinalSuffix,
//  formatClockTime, and the time-ago phrasing. This file is in the
//  widget's membershipExceptions list in project.pbxproj.
//
